...
class EmbeddingSimilarity(SimilarityStrategy):
    ...
    def compareWithTags(self, str1s, str2s, tags1, tags2):
        list1 = list(map(lambda tag: tag["ancestor"], tags1)) if tags1 is not None else []
        list2 = list(map(lambda tag: tag["ancestor"], tags2)) if tags2 is not None else []
        diff = set(list2).difference(set(list1))
        print("difference")
        print(diff)
        anc2 = sum(list(map(lambda tag: self.getWordEmb(tag), diff)))*2 if len(diff)!=0 else None
        #anc2 = sum(list(map(lambda tag: self.getWordEmb(tag["ancestor"]), tags2))) if tags2 is not None else None
        str1_descr = self.getWordEmb(" ".join(str1s).lower())
        str2_descr = self.getWordEmb(" ".join(str2s).lower()) - anc2 if anc2 is not None else self.getWordEmb(" ".join(str2s).lower())
        #tags1_descr = self.getWordEmb(" ".join(anc1).lower())
        #tags2_descr = self.getWordEmb(" ".join(anc2).lower()
        distance = cosine(str1_descr, str2_descr) 
        return (100-distance)/100
